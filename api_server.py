...
if __name__ == '__main__':
    ...
    # --- ПЕРЕОПРЕДЕЛЕНИЕ ФУНКЦИЙ ДЛЯ MONGODB ---

def load_terminals():
    global terminals
    try:
        terminals = {doc['terminal_id']: doc['data'] for doc in terminals_col.find({})}
        print(f"✅ [OVERRIDE] Загружено {len(terminals)} терминалов из MongoDB")
    except Exception as e:
        print(f"❌ Ошибка загрузки из MongoDB: {e}")

def save_terminals():
    for terminal_id, data in terminals.items():
        terminals_col.update_one(
            {"terminal_id": terminal_id}, 
            {"$set": {"terminal_id": terminal_id, "data": data}}, 
            upsert=True
        )
    print("✅ [OVERRIDE] Терминалы сохранены в MongoDB")

# Сделай то же самое для пользователей, если нужно
def load_users():
    global users
    users = {doc['username']: doc['data'] for doc in users_col.find({})}
    print(f"✅ [OVERRIDE] Загружено {len(users)} пользователей из MongoDB")
    # Перехватчик для сохранения терминалов
def save_terminals():
    for terminal_id, data in terminals.items():
        terminals_col.update_one(
            {"terminal_id": terminal_id}, 
...
# Перехватчик для загрузки пользователей
def load_users():
    global users
    users = {doc['username']: doc['data'] for doc in users_col.find({})}
    print(f"✅ [OVERRIDE] Загружено {len(users)} пользователей из MongoDB")

# Перехватчик для сохранения пользователей
def save_users():
    for username, data in users.items():
        users_col.update_one(
            {"username": username}, 
            {"$set": {"username": username, "data": data}}, 
            upsert=True
        )
    print("✅ [OVERRIDE] Пользователи сохранены в MongoDB")
